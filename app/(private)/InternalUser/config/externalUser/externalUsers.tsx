...
const filtrosUsuariosExternos: FiltroConfig[] = [
...
export function ExternalUsersContent() {
  const { externalUsers, setExternalUsers } = useUser()
  ...
  const handleSalvarUsuario = (e: React.FormEvent) => {
    ...
    const novoId = Math.max(0, ...externalUsers.map((u) => u.id)) + 1
